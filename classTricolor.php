...
class classTricolor
{
  ...
  function __construct($channel = -1, $day = -1, $kill_age_restriction = false, $LoadPrevDateTailFunc = true)
  {
    ...
    $this->CACHEDIR = $_SERVER['DOCUMENT_ROOT'] . dirname($_SERVER['SCRIPT_NAME']) . '/PROGCACHE_TRICOLOR/';
    ...
    $this->download_prog = $this->tempDate->format('d.m.Y');
  ...
  function Get()
  {
    global $DEFAULT_TZ;
    global $TIMESET;
    $ftime = filemtime('CHANNELS_LISTS/TRICOLOR.txt');
    if (@file_exists('CHANNELS_LISTS/TRICOLOR.txt') && ($ftime + $TIMESET) < time()) {
      require_once('Up.php');
      Up::tricolor('CHANNELS_LISTS/TRICOLOR.txt');
    }
    ...
    if (@file_exists($this->CACHEDIR . $this->channel . '.' . $this->download_date)) {
      ...
      $tent = @file_get_contents($CACHEDIR . 'CHANNELS_LISTS/TRICOLOR.txt');
      $r = "#\|" . $this->channel . "\|(.*?)\|#m";
      preg_match_all($r, $tent, $m1);
      $conts = self::get_cURL('https://m.tricolor.tv/ajax/program/grid.php?channel=select&type=list&channel-name=' . $this->channel . '&day=' . $this->download_prog, false);
      $contet = json_decode($conts, true);
      $cont = $contet['program'];
      preg_match_all('#data-hint-time="(\d+:\d+)\".*?data-hint-title=\"(.*?)\"#is', $cont, $TITLES);
      preg_match_all('# src="(.*?)\"#is', $cont, $LOGOS);
      count($TITLES[1]);
      $this->logo = "http://www.tricolor.tv" . $LOGOS[1][0];
    ...
    if ($this->channel <> 9999999) {
      ...
      if ($tok) {
        ...
        if (isset($matches[1][0])) {
          ...
          $tempTeleguide = new classTricolor($this->channel, $this->day - 1, $this->kill_age_restriction, false);
  ...
  public static function get_cURL($link, $headr = true)
  {
    $cookie = '';
    $ch = curl_init();
