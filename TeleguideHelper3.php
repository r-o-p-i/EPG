...
$Title = 'VSETV';
$REG = 40003;
$SCRIPT = 'TeleguideHelper3.php';
$CACHEDIR = $_SERVER['DOCUMENT_ROOT'] . $CURDIR . 'PROGCACHE_VSETV/';
$TEXT = 'CHANNELS_LISTS/' . $Title . '.txt';
headerHtml($Title, $REG);
CleanHtml($CACHEDIR);
if (isset($pass1)) {
  if ($pass1 === $SERVICE_PASS) {
    Up::dream('http://www.vsetv.com/channels.html', '/\<option value=channel_([0-9]{1,})\>([^\<]{1,})\<\/option>/', $TEXT);
    if (@file_exists($TEXT)) {
...
if (isset($_POST['channel'])) {
  if (file_exists($_SERVER['DOCUMENT_ROOT'] . $CURDIR . "LOGOS/" . $REG . "/" . $channels[$_POST['channel']][2] . ".png")) {
    $Logo = "http://" . $_SERVER['SERVER_NAME'] . $CURDIR . "LOGOS/" . $REG . "/" . $channels[$_POST['channel']][2] . ".png";
  }
  if (isset($Logo)) {
  ...
  $cont3 = "";
  if ($age_restr == 1) {
    $Teleguide = new classVSETV($channels[$_POST['channel']][2], -1, true);
  } else {
    $Teleguide = new classVSETV($channels[$_POST['channel']][2], -1, false);
  }
  $cont3 = $Teleguide->Get();
